...
load('HINF.mat')
 ...
 HINF.metric = metric;
 HINF.Wt_magdb = Wt_magdb;
 HINF.Ws_magdb = Ws_magdb;
...
Hmk3_RSLQR.ip = ip;
Hmk3_RSLQR.T = T_st(Hmk3_RSLQR.ip,:);
Hmk3_RSLQR.S = S_st(Hmk3_RSLQR.ip,:);
Hmk3_RSLQR.sr = sr;
Hmk3_RSLQR.rd = rd;
...
Hmk5_RSLQR.y = y;
Hmk5_RSLQR.Lu = Lu;
Hmk5_RSLQR.metric = metric;
Hmk5_RSLQR.ip = ip;
Hmk5_RSLQR.T = T_st(Hmk5_RSLQR.ip,:);
Hmk5_RSLQR.S = S_st(Hmk5_RSLQR.ip,:);
Hmk5_RSLQR.sr = sr;
Hmk5_RSLQR.rd = rd;
Hmk5_RSLQR.dele_Az = dele_Az;
Hmk5_RSLQR.dele_q = dele_q;
Hmk5_RSLQR.deledot_Az = deledot_Az;
Hmk5_RSLQR.deledot_q = deledot_q;  
...
Hmk5_SPC.y = y;
Hmk5_SPC.Lu = Lu;
Hmk5_SPC.metric = metric;
Hmk5_SPC.ip = ip;
Hmk5_SPC.T = T_st(Hmk5_SPC.ip,:);
Hmk5_SPC.S = S_st(Hmk5_SPC.ip,:);
Hmk5_SPC.sr = sr;
Hmk5_SPC.rd = rd;
Hmk5_SPC.dele_Az = dele_Az;
Hmk5_SPC.dele_q = dele_q;
Hmk5_SPC.deledot_Az = deledot_Az;
Hmk5_SPC.deledot_q = deledot_q;
Hmk5_SPC.y = y_spc;
% Scale the linear response by 32.174 for "one gee"
% Plot the time histories
figure('Name','Acceleration Time History')
plot(HINF.t,HINF.y(:,1),'b',Hmk3_RSLQR.t, Hmk3_RSLQR.y(:,1),Hmk5_RSLQR.t, Hmk5_RSLQR.y(:,1), Hmk5_SPC.t, Hmk5_SPC.y(:,1),'k','LineWidth',2);grid
legend(['Hinf 63% Tr = ' num2str(HINF.metric(5)) ' 95% Ts = ' num2str(HINF.metric(6))],...
       ...
       ['Hmk5 63% Tr = ' num2str(Hmk5_RSLQR.metric(5)) ' 95% Ts = ' num2str(Hmk5_RSLQR.metric(6))],...
       ['SPC 63% Tr = ' num2str(Hmk5_SPC.metric(5)) ' 95% Ts = ' num2str(Hmk5_SPC.metric(6))],'Location','Best');
xlabel('Time (sec)');
ylabel('Az (fps2)');
if(save_plots == 1) saveppt2(plot_file_name); end

figure('Name','Pitch Rate Time History')
plot(HINF.t,HINF.y(:,3)*rtd,'b',Hmk3_RSLQR.t, Hmk3_RSLQR.y(:,3)*rtd,Hmk5_RSLQR.t, Hmk5_RSLQR.y(:,3)*rtd, 'k','LineWidth',2);grid
legend('Hinf SF ', ...
       'Hmk3 RSLQR ',...
       'Hmk5 RSLQR',...
       'Location','Best');
...
plot(t,HINF.y(:,4)*rtd,'b',Hmk3_RSLQR.t, Hmk3_RSLQR.y(:,4)*rtd,Hmk5_RSLQR.t, Hmk5_RSLQR.y(:,4)*rtd,'k','LineWidth',2);grid
legend('Hinf SF ', ...
       'Hmk3 RSLQR ',...
       'Hmk5 RSLQR',...
       'Location','Best');
...
plot(t,HINF.y(:,5)*rtd,'b',Hmk3_RSLQR.t, Hmk3_RSLQR.y(:,5)*rtd,Hmk5_RSLQR.t, Hmk5_RSLQR.y(:,5)*rtd,'k','LineWidth',2);grid
legend('Hinf SF ', ...
       'Hmk3 RSLQR ',...
       'Hmk5 RSLQR',...
       'Location','Best');
...
real(squeeze(Hmk3_RSLQR.Lu)),imag(squeeze(Hmk3_RSLQR.Lu)),...
real(squeeze(Hmk5_RSLQR.Lu)),imag(squeeze(Hmk5_RSLQR.Lu)),...
real(squeeze(Hmk5_SPC.Lu)),imag(squeeze(Hmk5_SPC.Lu)),...
'k','LineWidth',2);grid
axis([-2 2 -2 2]);
legend('Unit Circle','Hinf-SF','Hmk3 RSLQR','Hmk5 RSLQR','Hmk5 SPC','Location','Best');
xlabel('Re(L)')
ylabel('Im(L)')
...
semilogx(HINF.w,20*log10(abs(squeeze(HINF.Lu))),'b',...
    Hmk3_RSLQR.w,20*log10(abs(squeeze(Hmk3_RSLQR.Lu))),...
    Hmk5_RSLQR.w,20*log10(abs(squeeze(Hmk5_RSLQR.Lu))),...
    Hmk5_SPC.w,20*log10(abs(squeeze(Hmk5_SPC.Lu))),...
    'k','LineWidth',2);grid
legend([ 'Hinf-SF LGCF = '     num2str(HINF.metric(4)) ],...
       [ 'Hmk3 RSLQR LGCF = '  num2str(Hmk3_RSLQR.metric(4)) ],...
       [ 'Hmk5 RSLQR LGCF = '  num2str(Hmk5_RSLQR.metric(4)) ],...
       [ 'Hmk5 SPC LGCF = '  num2str(Hmk5_SPC.metric(4)) ],...
       'Location','Best');
...
semilogx(HINF.w,20*log10(abs(HINF.rd)),'b',Hmk3_RSLQR.w,20*log10(abs(Hmk3_RSLQR.rd)),...
    Hmk5_RSLQR.w,20*log10(abs(Hmk5_RSLQR.rd)),Hmk5_SPC.w,20*log10(abs(Hmk5_SPC.rd)),'k','LineWidth',2);grid
legend([' Hinf min(I+Lu) = ' num2str(HINF.metric(2))],...
       ['Hmk3 RSLQR I+Lu min = ' num2str(Hmk3_RSLQR.metric(2)) ],...
       ['Hmk5 RSLQR I+Lu min = ' num2str(Hmk5_RSLQR.metric(2)) ],...
       ['Hmk5 SPC I+Lu min = ' num2str(Hmk5_SPC.metric(2)) ],...
       'Location','Best');
xlabel('Frequency (rps)')
ylabel('Mag dB')
...
semilogx(HINF.w,20*log10(abs(HINF.sr)),'b',Hmk3_RSLQR.w,20*log10(abs(Hmk3_RSLQR.sr)),...
        Hmk5_RSLQR.w,20*log10(abs(Hmk5_RSLQR.sr)),Hmk5_SPC.w,20*log10(abs(Hmk5_SPC.sr)),'k', 'LineWidth',2);grid
legend([' Hinf min(I+Lu) = ' num2str(HINF.metric(3))],...
       ['Hmk3 RSLQR I+Lu min = ' num2str(Hmk3_RSLQR.metric(3)) ],...
       ['Hmk5 RSLQR I+Lu min = ' num2str(Hmk5_RSLQR.metric(3)) ],...
       ['Hmk5 SPC I+Lu min = ' num2str(Hmk5_SPC.metric(3)) ],...
       'Location','Best');
xlabel('Frequency (rps)')
ylabel('Mag dB')
...
semilogx(HINF.w,HINF.T_Az,'b',HINF.w,Wt_magdb,'g',Hmk3_RSLQR.w,Hmk3_RSLQR.T',...
    Hmk5_RSLQR.w,Hmk5_RSLQR.T',Hmk5_SPC.w,Hmk5_SPC.T','k', 'LineWidth',2);grid
legend(['||T||inf = ' num2str(HINF.metric(11)) ' (dB)'],...
        'invWt',...
        ['Hmk3 RSLQR Tmax = ' num2str(Hmk3_RSLQR.metric(11)) ],...
        ['Hmk5 RSLQR Tmax = ' num2str(Hmk5_RSLQR.metric(11)) ],...
        ['Hmk5 SPC Tmax = ' num2str(Hmk5_SPC.metric(11)) ],...
        'Location','Best');
title('Comp Sens T');
...
figure('Name','Sens S');
semilogx(HINF.w,HINF.S_Az,'b',HINF.w,HINF.Ws_magdb,'g',Hmk3_RSLQR.w,Hmk3_RSLQR.S,...
    Hmk5_RSLQR.w,Hmk5_RSLQR.S,Hmk5_SPC.w,Hmk5_SPC.S,'k', 'LineWidth',2);grid
legend(['||S||inf = ' num2str(HINF.metric(12)) ' (dB)'],...
        'invWs',...
        ['Hmk3 RSLQR Smax = ' num2str(Hmk3_RSLQR.metric(12)) ],...
        ['Hmk5 RSLQR Smax = ' num2str(Hmk5_RSLQR.metric(12)) ],...
        ['Hmk5 SPC Smax = ' num2str(Hmk5_SPC.metric(12)) ],...
        'Location','Best');
title('Sens S');
xlabel('Freq (rps)');ylabel('Mag (dB)');
if(save_plots == 1) saveppt2(plot_file_name); end   
